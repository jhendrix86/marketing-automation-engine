...
def _apply_tenant_filtering(orm_execute_state):
    ...
    pulls in app.models.campaign etc., which import Base from this
    ...
    tenant_id is resolved here, once, into a plain closure variable
    before building the criteria lambda - SQLAlchemy's lambda-SQL caching
    also forbids invoking a function (e.g. get_tenant_context()) from
    inside a with_loader_criteria callable, since it normally extracts
    bound values without calling the lambda body at all.
    """
...
async def init_db():
    ...
    try:
        async with engine.begin() as conn:
            ...
            from app.models import tenant, campaign, email_campaign, social_post, lead, segment
