...
@pytest.mark.asyncio
@respx.mock
async def test_bearer_key_with_identity_sets_tenant_context_and_scopes_leads(client):
    ...
    same_tenant = await client.get(f"/leads/{lead_id}", headers={"Authorization": "Bearer key_for_tenant_a"})
    assert same_tenant.status_code == 200

    other_tenant = await client.get(f"/leads/{lead_id}", headers={"Authorization": "Bearer key_for_tenant_b"})
...
async def test_bearer_key_without_identity_falls_back_to_x_tenant_id(client):
    ...
    listing = await client.get(
        "/leads/", headers={"Authorization": "Bearer some_valid_key", "X-Tenant-ID": tenant_id}
    )
